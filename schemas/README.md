...
- Every record declares its schema URI, record type, schema version, producing framework version, timestamps, status, confidence, privacy, evidence references, human notes, extensions, and fictional marker.
- Relationships use stable IDs rather than embedding duplicate records.
- Human consent, restrictions, and dispositions use append-only decision histories. Each decision records a stable decision ID, accountable person ID, time, reason, and the exact prior decision it supersedes; current consent status matches the latest consent decision.
...
- Semantically typed references resolve to their declared record types or documented canonical unions. Outreach approval rejects active opt-out, requires the channel in recipient preferences, allows unknown consent only for bounded contextual human judgment, rejects controlling wait/do-not-contact, and requires a deciding person authorized by the linked practitioner profile.
...
The current portable record version is **3.0.0**. Records from the initial 1.0.0 draft first require the [schema 1-to-2 migration](../project/migrations/portable-records-1-to-2.md); 2.0.0 release-candidate records then require the [schema 2-to-3 migration](../project/migrations/portable-records-2-to-3.md). Changing only the version string is prohibited.
